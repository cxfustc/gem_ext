...
const char base2int_tbl_acgt[128] = {
	4,4,4,4, 4,4,4,4, 4,4,4,4, 4,4,4,4,
	4,4,4,4, 4,4,4,4, 4,4,4,4, 4,4,4,4,
	4,4,4,4, 4,4,4,4, 4,4,4,4, 4,4,4,4,
	4,4,4,4, 4,4,4,4, 4,4,4,4, 4,4,4,4,
...
gzFile
err_ckgzopen (const char * file, const char * mode,
		const char * c_func, const char * c_file, int line_c)
{
  gzFile fp;

  if ((fp = gzopen(file,mode)) == NULL)
		err_mesg ("[%s] fail to open file '%s' in file %s at line %d!",
				c_func, file, c_file, line_c);
...
size_t
err_ckfread (void * ptr, size_t size, size_t n_elem, FILE * fp,
    const char * c_func, const char * c_file, int line_c)
{
	size_t ret;

	if ((ret = fread(ptr,size,n_elem,fp)) != n_elem)
		err_mesg ("[%s] fail to call fread in file %s at line %d: %s!",
				c_func, c_file, line_c, strerror(errno));
...
void
send_work_signal (int * signals, int work_signal, int wait_signal, int n_thread)
{
  int i;

  for (i=0; i<n_thread; ++i)
    signals[i] = work_signal;

  for (;;) {
    for (i=0; i<n_thread; ++i)
      if (signals[i] != wait_signal)
        break;
...
int
time_parse (char * str, int32_t * h, int32_t * m, int32_t * s)
{
	char * ch;
...
void
file2new_dir (char * new_path, const char * new_dir, const char * old_path)
{
  const char * ch;

  if ((ch = strrchr(old_path,'/')) == NULL)
    ch = old_path;
  else
    ch += 1;

  sprintf (new_path, "%s/%s", new_dir, ch);
}

char *
get_necessary_param (int argc, char ** argv, int opt_idx)
{
  if (opt_idx >= argc)
    err_mesg ("fail to get necessary parameter!");

  return strdup (argv[opt_idx]);
}
