...
#define BMP_DEF2(SCOPE, name, type_t) \
	...
	SCOPE void bmp_blk_clear_##name ( \
      ...
      memset (blk->datas, 0, blk->m*sizeof(type_t)); \
    } \
		blk->n = 0; \
	} \
	SCOPE bmp_##name##_t * bmp_init_##name ( \
		...
		bmp->head = bmp->tail = bmp_blk_init_##name (init_type_f, blk_size); \
		return bmp; \
	} \
	...
	SCOPE type_t * bmp_alloc_##name ( \
		...
		blk = bmp_blk_init_##name (bmp->init_type_f, bmp->blk_size); \
		bmp->tail->next = blk; \
    blk->prev = bmp->tail; \
		bmp->tail = blk; \
		return blk->datas + blk->n++; \
	} \
  SCOPE void bmp_pop_##name ( \
      bmp_##name##_t * bmp) { \
    bmp_blk_##name##_t * blk; \
    blk = bmp->tail; \
    --blk->n; \
  ...
  \
  SCOPE void bmp_copy_##name ( \
      bmp_##name##_t * dst, \
      bmp_##name##_t * src, \
      void (*copy_type_f)(type_t*,type_t*)) { \
    type_t * dst_ptr; \
    type_t * src_ptr; \
    if (dst->n_item!=0 || dst->tail!=dst->head) \
      err_mesg ("destination bmp must be cleared first!"); \
    if (bmp_iter_init_##name(src) != 0) \
      err_mesg ("fail to init iteratio for souce bmp!"); \
    while ((src_ptr = bmp_iter_next_##name(src)) != NULL) { \
      dst_ptr = bmp_alloc_##name (dst); \
      if (copy_type_f == NULL) \
        memcpy (dst_ptr, src_ptr, sizeof(type_t)); \
      else \
...
#define bmp_init(name,blk_size,func) bmp_init_##name((blk_size),(func))
...
#define bmp_copy(name,dst,src) bmp_copy_##name((dst),(src))
